...
def generate_numbers(num, size, n, d):
    global S
    ...
    for i in range(0, 10):
        ...
        if (d % 2 == 0):
            if (num[0] == '0'):
                if (i != 0):
                    generate_numbers(str(i) + num, size + 1, n, d)
                    continue
            elif (num[-1] == str(d)[0] and i % 2 == 1):
                generate_numbers(num + str(i), size + 1, n, d)
                continue
        ...
        for j in range(len(num) + 1):
            ...
            generate_numbers(temp, size + 1, n, d)
...
for t in range(T):
    ...
    if (d % 2 == 1 and is_prime(int(str(d) * n))):
        print(str(d) * n)
        continue

    for i in range(n - 2 if d == 0 else n - 1, 0, -1):
        num = str(d) * i
        generate_numbers(num, i, n, d)
